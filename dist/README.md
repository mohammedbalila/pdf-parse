...
This package includes TypeScript type definitions. You can use it in your TypeScript projects as follows:
...
pdfParse(dataBuffer, {
  pagerender: customRenderFunction, // (optional) custom page renderer
  max: 5, // (optional) process only first 5 pages
  version: 'v1.10.100' // (optional) specify pdf.js version
})
  .then(function(data) {
    // Number of pages
    console.log(data.numpages);
...
See the `examples/typescript-example.ts` file for more detailed examples.
